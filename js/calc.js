...
export const BRUSH_PLANS = {
  ...
  mesquite: [
    { key: 'leaf', label: 'Leaf spray (mesquite under 8 ft)', rup: false, kind: 'mix', product: 'Sendero (clopyralid + aminopyralid)', altProducts: 'Remedy Ultra 0.5% + Reclaim 0.5%', pct: 1, pctRange: '1%', perGal: 4, method: 'ipt',
      when: 'Late spring to summer, once the soil is 75 °F at 12 in deep and the leaves have turned from light pea-green to a uniform dark green. Stop if drought curls the leaves or they yellow.',
      note: 'Spray until the leaves on every branch are wet, including the top and the far side, using an adjustable cone nozzle with coarse drops that reach the top of an 8-ft tree. Leaves drop within weeks, but judge rootkill the next summer: trees that don’t leaf out are dead.' },
    { key: 'stem', label: 'Stem spray (low-volume basal)', rup: false, kind: 'mix', product: 'Remedy Ultra (triclopyr ester)', altProducts: 'Pathfinder II ready-to-use, sprayed straight', pct: 25, pctRange: '15–25%', surfPct: 0, carrier: 'diesel or basal oil', perGal: 40, method: 'ipt',
      when: 'Any time of year. Not when the bark is wet, and not on stems with rough, thick bark.',
      note: 'Spray the lower 12 in of every stem all the way around until it is wet, but not to runoff. It works best on smooth-barked stems under about 4 in across. For multi-stem trees, treat every stem.' },
    { key: 'cutstump', label: 'Cut + stump spray', rup: false, kind: 'mix', product: 'Remedy Ultra (triclopyr ester)', pct: 25, pctRange: '15–25%', surfPct: 0, carrier: 'diesel or basal oil', perGal: 60, method: 'ipt',
      when: 'Any time of year. Spray each stump right after it is cut.',
      note: 'Cut low, then soak the cut surface, the bark on the sides of the stump and any exposed roots. Mesquite resprouts from buds on the root crown, so an unsprayed cut stump comes back multi-stemmed.' },
    { key: 'aerial', label: 'Aerial / broadcast (dense stands)', rup: false, kind: 'broadcast', product: 'Sendero', ptPerAcre: 1.75, carrier: 5, method: 'aerial',
      when: 'About 40–90 days after the leaves come out, when the soil is 75 °F at 12–18 in and the leaves are dark green.',
      note: 'Sendero at 28 oz/acre (1.75 pt) in at least 5 gal of water per acre, plus surfactant, is the most consistent aerial treatment in Texas trials (around 90% rootkill in good conditions). It spares most other woody plants. Hire an aerial applicator and plan for large blocks.' },
    { key: 'grub', label: 'Grub out below the bud zone', kind: 'none', method: 'grubbing',
      when: 'Any time of year. Moist soil makes pulling the roots easier.',
      note: 'The buds that resprout are on the root crown, in the top ~12 in of root. Grub, root-plow or skid-steer-grapple deep enough to take the whole crown, or the tree comes back. Good for scattered trees, and fine in hunting season.' },
    { key: 'fire', label: 'Prescribed fire', kind: 'none', method: 'fire',
      when: 'Winter burns with a burn plan and crew.',
      note: 'Fire top-kills small mesquite, but almost all of it resprouts. Use fire to suppress regrowth and keep grass ahead of brush between chemical or grubbing treatments.' },
  ],
  pear: [
